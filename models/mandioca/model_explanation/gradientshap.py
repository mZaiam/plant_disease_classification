...
for name, state_dict in models_to_process.items():
    ...
    if 'resnet' in name:
        model = ResNet(n_classes=3, fine_tuning=True, device=device).to(device)
    else:
        STUDY_NAME = 'mandioca_baseline' if name == 'baseline' else 'plantvillage'
        ...
        if name == 'baseline':
            ...
            model = CNNTransferLearning(best_trial, n_classes=3, device=device, fine_tuning=True).to(device)
