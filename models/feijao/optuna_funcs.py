...
def cross_validation(study, dataset, train_transforms, val_transforms, device, n_splits=10, top=50):
    ...
    for num, trial in enumerate(best_trials):
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
            ...
            train_loader = DataLoader(train_subset, batch_size=4, sampler=sampler, drop_last=True)
...
def cross_validation_resnet(dataset, train_transforms, val_transforms, device, n_splits=10, epochs=30, n_classes=4, fine_tuning=False):
    '''
    Cross-validation.
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        train_loader = DataLoader(train_subset, batch_size=4, sampler=sampler)
        val_loader = DataLoader(val_subset, batch_size=4, shuffle=False)
...
def cross_validation_tl(best_trial, dataset, train_transforms, val_transforms, device, n_splits=10, epochs=30, n_classes=4, fine_tuning=False):
    '''
    Cross-validation.
