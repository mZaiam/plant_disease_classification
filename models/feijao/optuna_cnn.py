...
from optuna_funcs import load_data, objective, cross_validation
...
study = optuna.create_study(
    ...
    storage=f'sqlite:///hp_tuning/{STUDY_NAME}.db',
    load_if_exists=True
)
...
if len(df) == 0:
    ...
    num_trials = TRIALS - len(df.loc[(df["state"] == "COMPLETE") | (df["state"] == "PRUNED")])
study.optimize(partial_objective, n_trials=num_trials)

# Cross-Validation

best_trial, loss, number = cross_validation(
    study, 
    train_val_dataset,
    train_transforms,
    val_transforms,
...
df_model = pd.DataFrame.from_dict(number, orient="index")

for model in loss.keys():
    for i in range(0, 10):
        df_model.loc[model, f"Cross_Val_{i}"] = \
            loss[model][i]

df_model.to_csv(f"cross_validation/{STUDY_NAME}.csv")
